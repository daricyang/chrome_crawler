...
var tencentPeople=function(data,callback){
	...
	var fans_count=data.html.match(/\"count\"\:.*?\}/);
	for(var i=0;i<names.length;i++){
		var name=names[i].replace('/\"name\"\:\"(.*?)\"/g','$1');
		var following=fans_count[i].replace('/\"following\"\:\"(.*?)\"/g','$1');
		var follower=fans_count[i].replace('/\"follower\"\:\"(.*?)\"/g','$1');
		...
		function(results){
			results.forEach(function(u){
				...
				for(var j=1;j<=Math.ceil(parseInt(u.following)/15.0);j++){
					next.push('http://1.t.qq.com/asyn/following.php?u='+u[name]+'&&time=&page='+j+'&id=&apiType=4&apiHost=http%3A%2F%2Fapi.t.qq.com&_r=1365666653702');
				}
				for(var j=1;j<=Math.ceil(parseInt(u.follower)/15.0);j++){
					next.push('http://1.t.qq.com/asyn/follower.php?u='+u[name]+'&&time=&page='+j+'&id=&apiType=4&apiHost=http%3A%2F%2Fapi.t.qq.com&_r=1365666653702');
				}
			});
		}	
	);
